...
int EncodeData( char *lpsSource)
{
	...
	nEncodeVersion = GetEncodeVersion(nVersion, lpsSource, ncLength);//�������������Ҫ�İ�?
...
int  PrintQrcode(char* str, u32 msize, u32 alignment, u32 lMargin,u8 * outbuf)
{
	...
	memset(m_byModuleData, 0, MAX_MODULESIZE * MAX_MODULESIZE);
	
	iRet = EncodeData(str);
	//uart_debug("EncodeData:%d\r\n", iRet);
	if(iRet < 0) return iRet;
	qr_w = iRet;
	for(relmsize = msize; relmsize > MIN_MSIZE; relmsize--)
	{
		if(relmsize*qr_w < (MAX_W_SIZE - lMargin)) break; 
	}
	if(relmsize < MIN_MSIZE) return -4;
	if(ALIGN_CENTER == alignment)
		left = (MAX_W_SIZE - relmsize*qr_w - lMargin)/2 + lMargin;
	else if(ALIGN_RIGHT == alignment) left = MAX_W_SIZE - relmsize*qr_w + lMargin;
	else left = lMargin;
	...
	for(i = 0; i < qr_w; i++)
	{
		...
		for(j = 0; j < qr_w; j++)
		{
			if(m_byModuleData[i][j])
			{
				for(k = 0; k < relmsize; k++)
				{
					*pdst |= 0x01 << (7 - bitOffset);
					bitOffset++;
					if (bitOffset == 8)
					{
						bitOffset = 0;
		...
		for(k = 0; k < relmsize; k++)
		{
			//PrinterWriteBitmap(buf, 1);
			memcpy(outbuf+dataoff,buf,48);
			dataoff +=48;
		}
	}
