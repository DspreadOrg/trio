                       ...
                       int fmt)
{
    ...
    else if(fmt == SPNG_FMT_RGB565)
    {
        for(; i < width; i++)
        {
            ...
            px[0] = (((plte[entry].green) >> 2) << 5) | ((plte[entry].blue) >> 3);
            px[1] = (((plte[entry].red) >> 3) << 3) | (((plte[entry].green) >> 2) >> 3);
        }
    }
}
...
int spng_decode_scanline(spng_ctx *ctx, void *out, size_t len)
{
    ...
    else if(fmt == SPNG_FMT_RGB565) pixel_size = 2;
    ...
    for(k=0; k < width; k++)
    {
        ...
        else if(ihdr->color_type == SPNG_COLOR_TYPE_INDEXED)
        {
            ...
            if(ihdr->bit_depth == 8)
            {
                if(fmt & (SPNG_FMT_RGBA8 | SPNG_FMT_RGB8 | SPNG_FMT_RGB565))
                {
            ...
            else if(fmt & (SPNG_FMT_RGB565))
            {
                ...
                pixel[0] = (((plte[entry].green) >> 2) << 5) | ((plte[entry].blue) >> 3);
                pixel[1] = (((plte[entry].red) >> 3) << 3) | (((plte[entry].green) >> 2) >> 3);
                continue;
            }
            else /* RGBA16 */
            {
        ...
        else if(fmt & (SPNG_FMT_RGB565))
        {
            ...
            pixel[0] = ((g_8 >> 2) << 5) | (b_8 >> 3);
            pixel[1] = ((r_8 >> 3) << 3) | ((g_8 >> 2) >> 3);
        }
        else if(fmt == SPNG_FMT_RGBA16)
        {
            if(ihdr->bit_depth != 16)
...
int spng_decode_row(spng_ctx *ctx, void *out, size_t len)
{
    ...
    else if(ctx->fmt == SPNG_FMT_RGB565) pixel_size = 2;
...
int spng_decode_image(spng_ctx *ctx, void *out, size_t len, int fmt, int flags)
{
    ...
    if(ihdr->bit_depth == 16 && fmt & (SPNG_FMT_RGBA8 | SPNG_FMT_RGB8 | SPNG_FMT_RGB565))
...
void spng_ctx_free(spng_ctx *ctx)
{
    ...
    if(!ctx->user.exif && ctx->exif.data != NULL) spng__free(ctx, ctx->exif.data);

    if(!ctx->user.iccp && ctx->iccp.profile != NULL) spng__free(ctx, ctx->iccp.profile);
...
int spng_decode_image_ex(spng_ctx *ctx, void *out, size_t len, int fmt, int flags,  void *check_fun)
{
    int (*func)(void);
    func = (int (*)(void))(check_fun);
